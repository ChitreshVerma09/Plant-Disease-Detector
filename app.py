...
TREATMENTS = {
    'Potato___Early_blight': {
        ...
        'color': '#f59e0b',
        'desc': 'Early Blight is caused by Alternaria solani fungus.',
        'remedy': '✂️ Remove infected leaves.<br>🧪 Spray Copper-based Fungicide every 7-10 days.'
    },
    'Potato___Late_blight': {
        'status': 'Critical Disease Detected 🚨',
        'color': '#ef4444',
        'desc': 'Late Blight is caused by Phytophthora infestans.',
        'remedy': '⚠️ Apply systemic fungicide like Ridomil Gold immediately.<br>🔥 Destroy severely affected plants.'
    },
    'Potato___healthy': {
        'status': 'Healthy Plant 🌱',
        'color': '#22c55e',
        'desc': 'Your plant leaves are healthy!',
        'remedy': '✅ Maintain regular watering and fertilizing schedule.'
    }
}
...
st.markdown('<div class="header-subtitle">Upload a potato leaf photo for real-time AI disease prediction</div>', unsafe_allow_html=True)
...
with col2:
    if uploaded_file is not None and model is not None:
        ...
        info = TREATMENTS.get(raw_class_name, {'status': 'Complete', 'color': '#22c55e', 'desc': '', 'remedy': 'Consult specialist.'})
        clean_name = raw_class_name.replace("Potato___", "").replace("_", " ").title()

        st.markdown(f"""
            <div class="result-card" style="border-left-color: {info['color']};">
                <span style="color: {info['color']}; font-weight: bold;">{info['status'].upper()}</span>
                <div class="disease-title">{clean_name}</div>
                <p style="color: #94a3b8;">{info['desc']}</p>
            </div>
        """, unsafe_allow_html=True)
        ...
        st.markdown(f"""
            <div class="treatment-box">
                <h4 style="color: #22c55e;">💡 Recommended Treatment:</h4>
                <p>{info['remedy']}</p>
            </div>
        """, unsafe_allow_html=True)
    else:
